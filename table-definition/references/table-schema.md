...
derived_from: <原本のファイル名>  # もとにした定義書。口頭説明なら「口頭」
...
| `derived_from` | もとにした定義書のファイル名。口頭説明から作った場合は「口頭」 |
...
**書かないもの**: 件数・欠損率・最小・最大などの数値プロファイルと、値のサンプル。定義書に載っていても、注意点に定性で書く。分からない項目は空欄にせず「要確認」と書く。

## wiki「データ」表の型

`docs/wiki/index.md` に「データ」節を置き、1 論理テーブル 1 行。データ列から YAML へリンクする。
