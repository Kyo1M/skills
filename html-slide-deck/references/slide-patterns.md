...
構成 md（deck-outline）の「図版指示」にはここにある **型の名前** と **アイコンの概念名**（`icons.md` の対応表）を書いてもらう。書かれていないときは、この表で選ぶ。
...
| 3〜4 本の柱 | 06 提供価値（3 軸） | 行ごとにアイコン＋見出し＋一文 |
| 選択肢の比較（依頼時のみ） | 07 比較表 | 推奨行に Klein Blue |
| 手順・段階 | 08 4 ステップ | 横並びのステップカード。アイコン付き |
...
### 01 表紙（`data-label="表紙"`）
- 用途: 資料の顔。本日の流れ。
- 構造: 左サイドバー 520px（青い正方形 52px、資料名 3 行、日付・区分）｜右: kicker（英字ラベル・日付）→ 明朝タイトル 74〜106px → ルール線 80×5 → リード文 29px（1〜3 行）→「本日の流れ」番号付き 4〜6 項目。
- アイコン: 流れの各項目に 28px で付けてよい（置く場合は全項目に）。
- 注意: 定例の大タイトルは定例名そのもの。kicker の区切りは `·` か空白（`──` 不可）。区分は「Internal」「Confidential」等の 1 語。

### 02 目次（`目次`）
- 構造: 2 列グリッド。各行は `90px 1fr` で大きな番号（明朝 `#A9AAAD`）＋章名（太字）＋一文。
- アイコン: 各章に 1 個（章扉と同じ概念）。
...
- アイコン: 各ステップ 1 個（3〜5 個）。現在のステップだけ Klein Blue。
...
- 構造: ヘッダ（月＋週、`今週` マーク）→ トラック見出し行（`#F4F2EE`）→ タスク行（ラベル列＋`repeat(N,1fr)` に帯）。週の縦線はオーバーレイ 1 枚。実施済みは緑帯＋「✓ 済」。
- アイコン: トラック名の左に各 1 個可。
- 注意: 帯の行に `padding`・`gap` を入れない。ラベル `line-height:1.25`。トラック名は「実装／仕組みづくり」（スラッシュ）。5 分・週単位で十分。
...
### 14 進捗報告（`進捗報告`）
- 構造: 右上「報告」バッジ。行ごとに `520px 80px 1fr` = 前回決めたこと｜→｜実績。先頭に ✓。末尾に「本日はこれを扱う」の一文。
- 注意: 定例の【振り返り】に使う。「先週宣言したこと vs 実績」の形はこの型。
...
- アイコン: 行グループ名（仕組み＝`ic-gear`、利用＝`ic-people`）と、ステップ名の上に各 1 個（工程番号の代わりにしてもよい）。1 枚に 8 個を超えるならステップ側のアイコンは省く。
...
<div style="border:1px solid #D8D6D0; display:grid; grid-template-columns:242px repeat(7,1fr);">
  ...
  <div style="background:#F7FAFF; border-right:1px solid #E6E4DE; border-bottom:1px solid #E6E4DE; padding:20px 18px; display:flex; align-items:center; gap:12px;">
    <svg width="32" height="32" style="color:#002FA7; flex:none;"><use href="#ic-gear"/></svg>
  ...
  <div style="border-right:1px solid #E6E4DE; padding:20px 18px; display:flex; align-items:center; gap:12px;">
    <svg width="32" height="32" style="color:#2F3135; flex:none;"><use href="#ic-people"/></svg>
...
### E. ガント（型 11 の部品）
- `design-system.md` §8 の規定どおり。帯 = `grid-column:a/b` の塗り矩形、縦線 = オーバーレイ 1 枚、行に padding/gap なし。
