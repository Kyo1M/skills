...
置く: **見出し帯カード（部品 B）の見出しだけ**（章扉の章名も可）。
置かない: 箇条書きの各行・番号付きリストの各項目・表の行・行グループ名・ガントのトラック名・工程やステップのノード・表紙の「本日の流れ」・目次・文中・フッター・注意帯・タイトル横。
量: 1 枚 0〜4 個（0 個の枚があってよい。5 個以上は多すぎる）。色は本文色、強調 1 個だけ Klein Blue。概念と 1:1。
...
対応の取り方の例（定例資料の見出し帯カード）:
「AI が支援すること」= `ic-spark`、「ヒトが担うこと」= `ic-person`、「案件の進め方で起きていること」= `ic-flow`、「分析で起きていること」= `ic-chart`、「層 1 月次 KPI」= `ic-gauge`、「層 2 案件内の質的評価」= `ic-list`、「お伺いしたいこと」= `ic-chat`。

対応表の多くの概念（工程・トラック・進行表の行など）は、新しい置き方では出番が無い。表は消さずに残す（章扉や将来の見出し帯カードで使う）。
