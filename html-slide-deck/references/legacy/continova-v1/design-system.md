...
| `--iso-top` | `#FBFBFA` | アイソメ概念図の天面（最も明るい面） |
| `--iso-left` | `#E7E7E4` | アイソメ概念図の左面（中間トーン） |
| `--iso-right` | `#D5D5D1` | アイソメ概念図の右面（最暗トーン） |
| `--iso-line` | `#6F7175` | アイソメ立体の輪郭（pebble と同値） |
| `--iso-key-top/left/right` | `#EAF1FF / #C9D8F5 / #AFC4EF` | 主役ノードの面（Klein Blue 系・1図1ノードのみ） |
...
ページ役割バッジ（slide-patterns.md Pattern 14）は「議論ページがこのデッキの主役」という Klein Blue のシグナル規律に沿って配色する。バッジは小面積なので青面積規律（5-12%）を圧迫しない。

| 役割 | 配色 | 意味 |
|---|---|---|
| 【議論】 | Klein Blue 塗り + 白文字 | デッキの主役。参加者の反応を求めるページ |
| 【確認】 | `--brand-blue-soft` 地 + Klein Blue 文字 | 軽い合意を取るページ |
| 【報告】 | `--snow` 地 + `--stone` 文字 | 聞くだけでよいページ |
| 【参考】 | 白地 + グレー枠 + `--pebble` 文字 | Appendix の網羅ページ |

役割バッジに第2ブランドカラーや success/warning を使わない（役割と評価は別軸）。
...
- **節目スライドにアイソメトリック概念図を 1 枚置く**（Pattern 13: isometric-concept）: 表紙・章マップ・全体像・運用などの **節目（structural / concept）スライド** に、手書きのアイソメ概念図を主役として置くと「図の形」で記憶に残る。観点カードが密なスライドには置かない（文字を優先）。

> いずれも「足し算の装飾」ではなく「主従と構造の強調」。迷ったら、装飾を足すのではなく**主役を1つに絞って大きくする**。

### グレー面陰影は「形」であり装飾ではない（アイソメ概念図の扱い）

「記憶に残すために装飾（グラデ・影・色）を足すな」という原則と、アイソメ概念図の **グレー 2〜3 トーンの面** は矛盾しない。区別の基準は次のとおり:

- **形（構造）の表現 = OK**: アイソメ立体の天面/左面/右面を `--iso-top/left/right` の 3 トーンで塗り分けるのは、奥行き＝立体の「形」を示すための情報設計。装飾の影ではない。`#iso-shadow`（ガウスぼかし）による**ソフトな接地影**も「接地＝立体がそこに在る」ことの表現として可。`opacity` は 0.12 前後に抑える。
- **意味のないリッチ化 = NG**: 派手なグラデーション、複数色、ネオン、強いドロップシャドウ、立体の必然性がない 3D 化は従来どおり禁止。
- **第 2 ブランドカラーを増やさない**: 立体は基本グレー（インク系）の線画＋面。**Klein Blue は各図の主役ノード 1 つだけ**に使う（その図の主張＝「ここが核」を 1 点で示す）。複数ノードを青くして意味を薄めない。
- **線画の質感を揃える**: 輪郭ストロークは `--iso-line`、`stroke-width:1.4`、`stroke-linejoin:round`（本文の線アイコン `.ic` と同じ質感）。ラベルは SVG に焼き込まず横書き（投影下限 12px）。
- 旧 `isometric-diagram`（codex 生成画像）は品質不安定で廃止済み。アイソメは **手書き SVG（Pattern 13）** で描く。
