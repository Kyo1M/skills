...
> **可読性の基準（2026-06 更新・底上げ版）**: 1672×941 の投影前提では、本文 14px 以下は潰れやすく「パッと見て伝わらない」。**Body text は 16px を既定**とし、15-17px の帯で運用する。チェックリスト・カード本文・注釈帯・コマンド帯もこの帯に合わせる（読み手が一番触れる本文を最優先で大きく）。Top message は **26px**。ツリー・プロンプト掲載ボックス等のモノスペース密度の高い要素は 15px まで、注釈・凡例・補助ラベル（SVG 図中ラベルを含む）は **12px を下限**とする。**11px 以下は投影で読めないため使わない**。表・マトリクスなど構造上どうしても詰まるスライドだけ、本文を **15px** まで（それ以下にはしない）下げてよい。「全情報を載せたい」場合は font を削るのではなく、密なスライドを 2 枚に分割するか 4 カラムを 2×2 に組み替える。
