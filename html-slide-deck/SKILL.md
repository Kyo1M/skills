...
- **基準値（2026-06 底上げ版）**: top-message **26px** / 本文 **16px**。読み手が一番触れる本文を大きく保ち「パッと見てそのスライドの主張が伝わる」を最優先にする
- **密度が高いスライド**（マトリクス・table・複数要素が並ぶ）: 本文を **15px** まで下げてよい（それ以下にはしない）。font を削る前に、まず **4 カラムを 2×2 に組み替える／スライドを 2 枚に分割する** 方を検討する
- **密度が低いスライド**（4 カード・3 ステップ・段階フロー等）: 本文 16-17px、要素 padding と高さを拡大してページ全体を使う
- **投影前提の下限**: 本文・チェックリスト・注釈帯・コマンド帯は **16px 既定（最低 15px）**、ツリー・プロンプト等のモノスペース密集要素は 15px まで、注釈・凡例・補助ラベル（SVG 図中ラベルを含む）は **12px を下限** とする。**11px 以下は使わない**
- **「全情報を載せたい」場合**: font を削って詰め込むのではなく、密なスライドを 2 枚に分割するか 4 カラムを 2×2 に組み替えて 16px を維持する
- **下部 callout を削除して余白が空いた場合**: 情報を追加するのではなく、**既存要素を伸ばす**（カード高さ・フォント・padding）。空白を空白のまま残すより、余裕を持って読める方が良い

ブラウザで「現状の文量に対して 1672×941 がどう埋まるか」を確認しながら微調整する（はみ出しは各スライドの `.body` の自然高さ ≤ 利用可能高さで機械チェックできる）。CSS では `.top-message.lg` `.body.lg` のような modifier クラスを用意し、密度低スライドに付与する運用が扱いやすい。
