...
- 13の再利用可能なスライドパターン(すべて手書きSVG・HTML/CSS)で設計効率化
...
3. frontmatter / ファイル名を変えた場合のみ `npm run lint:changed --prefix scripts` で検証（本文だけの修正はスキップ可）
...
- `references/self-review-checklist.md` — Phase 5 で通す 8 観点のセルフレビューチェックリスト
