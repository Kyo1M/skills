...
- **密度が高いスライド**（マトリクス・table・複数要素が並ぶ）: フォントサイズを基準値（top-message 22px / body 13-14px）で抑え、要素 padding を最小限に
- **密度が低いスライド**（4 カード・3 ステップ・段階フロー等）: 基準値より大きく（top-message 24px / body 14-15px）、要素 padding と高さを拡大してページ全体を使う
- **下部 callout を削除して余白が空いた場合**: 情報を追加するのではなく、**既存要素を伸ばす**（カード高さ・フォント・padding）。空白を空白のまま残すより、余裕を持って読める方が良い

ブラウザで「現状の文量に対して 1672×941 がどう埋まるか」を確認しながら微調整する。CSS では `.top-message.lg` `.body.lg` のような modifier クラスを用意し、密度低スライドに付与する運用が扱いやすい。
