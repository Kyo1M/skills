...
- 各スライドの 1 メッセージ、タイトル（副題なしの名詞句）、リード文（結論＋判断理由・結果 1〜3 行）、ボディの実文言、型、アイコンの概念名
...
- 20 型: 表紙／目次／自己紹介／章扉／現状と目指す姿／提供価値 3 軸／比較表／4 ステップ／推進体制／スケジュール（月帯）／ガント（タスク×週）／KPI／ロードマップ／進捗報告／確認事項／議論／まとめ／付録 早見表／お問い合わせ／**進行表**（仕組み／利用の 2 段）
- 文字だけの枚（箇条書きだけの軽い共有、論点を絞った議論）も可。主役の構造は置けるときに置く
- 選び方の指針:
  - 案件ごとの現在地 → 進行表（定例で更新し続ける）。前回決めたこと→実績 → 進捗報告
  - 手順・段階 → 4 ステップ（横）か縦フロー。3〜4 本の柱 → 提供価値 3 軸（アイコン付き）
  - 週単位の実績・予定 → ガント（1 枚に統合・実施済み ✓）。月単位 → スケジュール（月帯）
  - 1 論点の相談 → 議論（方針＝たたきを直接提示。比較は依頼時のみ）
  - 上のどれでもない → 箇条書き型（部品 A）
- アイコンは `references/icons.md` の対応表で概念と 1:1 に固定する（置く場所はカード・列の見出し／工程ノード／章扉・表紙の流れ・目次／表の行グループ名。各行には付けない。1 枚 3〜6 個）。対応表に無い概念は登録してから使う
...
9. 画像枠の指示文・プレースホルダ数値・PPTX 手直しの作り
...
- 脱ボックス（塗りは区切る意味があるものだけ・1 枚 2〜3 個）、グラデ・影・角丸なし、画像は破線枠＋指示文
- PPTX 手直し前提: 箇条書きは 1 テキストボックス、帯・カードは 1 つの塗り矩形、ガントの帯の行に padding/gap なし
...
- 「太字キーワード ── 短い説明」の 1 行圧縮、`──`・`―` の使用、タイトルの副題、「〜を、〜に。」構文、動詞締めのタイトル
- `data-speaker-notes` を付ける（発表者ノートは使わない）
- 箇条書きの各行・文中・タイトル横にアイコンを付ける／1 枚に 7 個以上／同じ概念に別のアイコン／絵文字・アイコンフォント
